# Legacy: JavaFX Desktop Chat (archived)

This is the original implementation: a JavaFX desktop client talking to a
Java-WebSocket server, with PostgreSQL storage. It's kept here, buildable and
untouched, as a reference while the project is rebuilt as a deployable web
app (see `../BE` and `../FE`). Don't add new features here — port them
forward into `BE`/`FE` instead.
...
All commands below run from inside `legacy/`.
